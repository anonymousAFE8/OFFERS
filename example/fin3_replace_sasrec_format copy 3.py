...
# 获取B的最后一个索引和用户ID
last_index_b = parsed_b[-1]["index"]
last_user_id_b = parsed_b[-1]["user_id"]
...
output = [{"index": entry['index'],
           ...
           "sequence_timestamps": entry['sequence_timestamps']} for entry in new_a_content]

# 将结果写入新的 CSV 文件
output_file_path = args.root_path+'/sasrec_format_final.csv'
...
ratings_output_path = os.path.join(args.root_path, 'ratings.csv')
