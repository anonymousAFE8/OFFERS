...
class EarlyStopping(object):
    ...
    def save_checkpoint(self, epoch): # TODO haddle saving checkpoint in ddp
        if self.save_dir is not None:
            ...
            _best_ckpt_path_wo_ckpt__ = self._best_ckpt_path.rsplit('/', 1)
            if len(_best_ckpt_path_wo_ckpt__) > 1:
                _best_ckpt_path_wo_ckpt = _best_ckpt_path_wo_ckpt__[0] + '/'
            else:
                _best_ckpt_path_wo_ckpt = ""
            torch.save(self.best_ckpt, os.path.join(self.save_dir, _best_ckpt_path_wo_ckpt+'pre-trained_embedding.ckpt'))
